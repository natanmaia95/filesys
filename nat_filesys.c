...
//for hugepage
#include <sys/mman.h>
#include <linux/mman.h> // For MAP_HUGETLB
...
int delete_file(FILE *fs, const char *name) {
...
int command_deleteFile(const char* disk_image, const char* name) {
    ...
    if (delete_file(fs, name) != 0) {
        printf("File '%s' Couldn't be deleted.\n", name);
        fclose(fs);
        return 1;
    }
...
int main(int argc, char *argv[]) {
    ...
    //     if (argc < 3) {
    //         printf("Usage: %s sort <filename>\n", argv[0]);
    //         return 1;
    //     }
    //     const char *filename = argv[2];
    //     return sort_file(disk_image, filename);
    // }
